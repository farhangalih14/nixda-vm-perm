...
void ImGui::ShadowText(const char* label, ImU32 text_col, ImU32 shadow_col, float shadow_thickness, const ImVec2& pos_text)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(3, (label_size.y / 2) + 1), bb.Max - ImVec2(3, (label_size.y / 2) - 1), shadow_col, shadow_thickness, ImVec2(0, 0), 100.f);
...
// Define una variable global para almacenar la posición anterior del desplazamiento
static std::map<ImGuiID, float> previous_scroll_v_map;
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const float grab_h_pixels = ImClamp(scrollbar_size_v * ((float)size_avail_v / (float)win_size_v) * 0.2f, style.GrabMinSize, scrollbar_size_v);
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        const float scroll_v_norm = ImSaturate((clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter - grab_h_norm) / (1.0f - grab_h_norm));

        // Apply interpolation for smooth scrolling
        float previous_scroll_v = previous_scroll_v_map[id];
        float current_scroll_v = scroll_v_norm * scroll_max;
        float interpolated_scroll_v = previous_scroll_v + (current_scroll_v - previous_scroll_v) * 0.1f; // 0.1f is the interpolation factor (adjust as needed)
        *p_scroll_v = (ImS64)interpolated_scroll_v;

        // Update previous_scroll_v_map
        previous_scroll_v_map[id] = interpolated_scroll_v;
    ...
    const ImU32 grab_col = GetColorU32(colors::MainColor);
...
void ImGui::ImageRotation(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImU32 tint_col, float speed) {
    ...
    rotation -= speed / ImGui::GetIO().Framerate * 60.f; // Rotar al otro lado
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        ImGui::GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(style.Colors[ImGuiCol_FrameBg]), style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            static std::map<ImGuiID, inputtext_state> filing;
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(
                    frame_bb.Min + ImVec2(it_filing->second.value + 0, 12),
                    frame_bb.Min + ImVec2(it_filing->second.value + 1, 30),
